...
class esas_commands():
   def Lightcurves(self,choice,inputfile,elow,ehigh): #make FOV or corner LCs
      ...
      if choice!="FOV":
         if choice!="corner":
             ...
             exit()
      if choice=="FOV":         
         outputfile="%s-LC-%s-%s.fits"%(CCD,elow,ehigh)
         if os.path.isfile(outputfile) == True:
            os.system("rm %s"%outputfile)
         if os.path.isfile("%s_gti.txt"%CCD) == True:
            os.system("rm %s_gti.txt"%CCD)
         if os.path.isfile("%s_gti.txt"%CCD) == True:
            os.system("rm %s_gti.fits"%CCD)
         os.system("evselect table=%s expression='(PATTERN<=12)&&(PI in [%s:%s])&&((FLAG & 0xfb0000) == 0)&&!((DETX,DETY) in BOX(10167,13005,3011,6575,0))' filtertype=expression rateset=%s timecolumn=TIME timebinsize=1 maketimecolumn=yes makeratecolumn=yes withrateset=yes &> tmp_command.csh"%(inputfile,elow*1000,ehigh*1000,outputfile))
         return str(outputfile)
      elif choice=="corner": 
         outputfile="%s-LC-corn-%s-%s.fits"%(CCD,elow,ehigh)
         if os.path.isfile(outputfile) == True:
            os.system("rm %s"%outputfile)
         if os.path.isfile("%s_gti.txt"%CCD) == True:
            os.system("rm %s_gti.txt"%CCD)
   ...
   def expr_ccdCheck(self,CCD,ccdNum,elow,ehigh):
       if CCD.find("mos1")>=0:
           expression="(PATTERN<=12)&&!((DETX,DETY) in BOX(10167,13005,3011,6575,0))&&(((FLAG & 0x766a0f63)==0)||((FLAG & 0x766a0763) == 0))&&(CCDNR==%s)&&(PI in [%s:%s])"%(ccdNum,elow*1000,ehigh*1000)
       elif CCD.find("mos2")>=0:
           expression="(PATTERN<=12)&&((FLAG & 0x766a0f63)==0)&&(CCDNR==%s)&&(PI in [%s:%s])"%(ccdNum,elow*1000,ehigh*1000)
       return expression
